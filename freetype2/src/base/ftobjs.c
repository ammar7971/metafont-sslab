...
#include FT_SERVICE_FOA_FACE_INFO_H  /* FOA service header file inclusion */
...
#define FT_COMPONENT  trace_objs
               ...
               FT_Face     *aface )
  {
    ...
    /* FOA check for METAFONT file */
    
	    char *File_name;
	    char *File_extension; 
	    File_name =  (char*)pathname;
	    FT_Module    foa_module = FT_Get_Module( library, "FOA" );
	    FT_Service_FOAInfo  FOA_service; /* FOA service object declaration */    
	    FOA_service = (FT_Service_FOAInfo)
	                    ft_module_get_service( foa_module,
	                                           FT_SERVICE_ID_FOA_CUSTOM_INFO,
	                                           0 );
		/* call FOA METAFONT finder module to check File extension */
	    File_extension = FOA_service->FOA_MF_Finder(File_name);
		printf("Font file path is %s \n", File_name);       
	    printf("Font File extension is %s \n", File_extension);
	    printf("MF font variable value is %d \n", MF_DONE);

    /* If File extension is METAFONT and this block of code is generated just once*/
        if ( FOA_service && strcmp( File_extension, "mf") == 0 && MF_DONE == 0)
        {
	  	/* CALLING FOA service */
		  printf("MF font variable value is %d \n", MF_DONE);
		  MF_DONE = 1;
		  printf("Calling FOA Module for processing METAFONT file \n \n");	
		  /* Call FOA module for getting converting METAFONT file to PFB file using mftrace */	
		  PFB_Font_file = FOA_service->FOA_MF_TO_PFB(File_name); 
		  printf("PFB font file path returned in base layer is %s \n", PFB_Font_file);
		  printf("Path of METAFONT font file is %s \n", pathname);
		  pathname = PFB_Font_file;
		  printf("Path of file after converting Metafont file to PFB file for TYPE1 driver module is %s \n", pathname);
		/* Ending FOA service */
		} 

    /* If File extension is GF or DVI and this block of code is generated just once
        if ( FOA_service && strcmp( File_extension, "2602gf") == 0 || strcmp( File_extension, "dvi") == 0 && MF_DONE == 0)
        {
	  	/* CALLING FOA service for GF or DVI files*/
		 /* printf("MF font variable value in GF,DVI service is %d \n", MF_DONE);
		  MF_DONE = 1;
		  printf("Calling FOA Module for processing GF,DVI file \n \n");	
		  /* Call FOA module for processing GF,DVI files */	
		  /*FOA_service->FOA_GF_DVI(File_name, File_extension); 
		/* Ending FOA service GF and DVI service*/
		/*}*/

        /* If FOA has been already called for converting PFB file than directly get the PFB file path*/
        if(PFB_Font_file)
        {
        	pathname = PFB_Font_file;
        	printf("Path name, if METAFONT file is called second time %s \n", pathname);
        }

    /* FOA check for METAFONT file endned */
