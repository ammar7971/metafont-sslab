...
#define X_TOO_LONG( x, size, display )                   \
        ...
        char*  argv[] )
  {
    ...
    start_point = clock();
	ST = (double) start_point;
	printf("start time is %f\n", ST );
	printf("Start Time just started \n");
    /* Initialize engine */
    handle = FTDemo_New();
    ...
    FTDemo_Update_Current_Flags( handle );
 /* Checking ending time of FOAw time */
	end_point = clock();

	timechk =  ((double) end_point - (double) start_point) / (double) 1000;
	ST = (double) start_point;
	ET = (double) end_point;
	printf("start time is %f\n", ST );
        printf("end time is %f\n", ET );
	printf("duration time of FOA module is %f\n", timechk );
